...
# Run Android APK Script
# This script installs and runs the Android APK on connected device/emulator
...
echo -e "${BLUE}📱 Running Android APK...${NC}"
...
if [ ! -f "$APK_PATH" ]; then
    ...
    echo -e "${YELLOW}💡 Run ./scripts/build-android.sh first to build the APK${NC}"
    exit 1
fi
...
PACKAGE_NAME="com.example"  # Default from the build.gradle we saw
